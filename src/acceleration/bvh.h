...
public:
    ...
    __device__ constexpr explicit BLAS(AccelerationNode<Primitive> *bvhTotalNodes, float totalArea, const float *cdf, const size_t _numPrimitives, AreaLight *emitter, BSDF *bsdf) noexcept
            : root(bvhTotalNodes), cdf(cdf), numPrimitives(_numPrimitives), bsdf(bsdf), emitter(emitter), totalArea(totalArea){
...
public:
    ...
    [[nodiscard]] __device__ bool rayIntersect(const Ray3f &_r, Intersection &rec, bool isShadowRay=false) const noexcept{
        Ray3f r = _r;
        Intersection record;
        bool hasHit = false;
