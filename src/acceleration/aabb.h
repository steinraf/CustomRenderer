...
struct AABB{
    ...
    __host__ __device__ constexpr AABB(const Vector3f &a, const Vector3f &b, const Vector3f &c) noexcept
        : min({CustomRenderer::min(a[0], CustomRenderer::min(b[0], c[0])),
               CustomRenderer::min(a[1], CustomRenderer::min(b[1], c[1])),
               CustomRenderer::min(a[2], CustomRenderer::min(b[2], c[2])),
              }),
          max({CustomRenderer::max(a[0], CustomRenderer::max(b[0], c[0])),
    ...
    [[nodiscard]] __device__ constexpr bool rayIntersect(const Ray &ray, float &nearT, float &farT) const noexcept {
